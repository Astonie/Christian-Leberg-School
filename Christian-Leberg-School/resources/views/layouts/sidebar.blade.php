<div class="fixed inset-y-0 left-0 z-50 flex flex-col w-64 bg-gradient-to-b from-slate-900 via-slate-900 to-slate-950 text-white min-h-screen transition-all duration-300 ease-in-out shadow-2xl transform lg:translate-x-0 lg:static lg:inset-0"
    ...
    <div class="flex items-center justify-between px-4 sm:px-6 h-16 lg:h-20 bg-slate-950/50 backdrop-blur-sm border-b border-slate-800/50">
        <div class="flex items-center space-x-3">
            <div class="w-9 h-9 lg:w-10 lg:h-10 bg-white rounded-xl flex items-center justify-center shadow-lg p-1.5">
                <img src="{{ asset('images/school_logo.png') }}" alt="{{ setting('site_name', 'School') }}" class="w-full h-full object-contain">
            </div>
            <div class="hidden sm:block">
                <h1 class="text-sm lg:text-base font-bold text-white leading-tight">{{ setting('site_name', config('app.name', 'School Portal')) }}</h1>
    ...
    <nav class="flex-1 px-2 sm:px-3 py-4 lg:py-6 space-y-1 overflow-y-auto scrollbar-thin scrollbar-thumb-slate-700 scrollbar-track-transparent">
        ...
        @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('head-teacher') || auth()->user()->hasRole('deputy-head-teacher') || auth()->user()->hasRole('teacher'))
            <!-- Students -->
            ...
            @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('head-teacher') || auth()->user()->hasRole('deputy-head-teacher') || auth()->user()->hasRole('head-teacher') || auth()->user()->hasRole('deputy-head-teacher'))
                <!-- Teachers -->
            ...
            @else
                ...
                <a href="{{ route('teacher-assessments.index') }}" class="group flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 {{ request()->routeIs('teacher-assessments*') ? 'bg-gradient-to-r from-blue-600 to-cyan-500 text-white shadow-lg shadow-blue-500/30' : 'text-slate-300 hover:bg-slate-800/50 hover:text-white' }}">
                    <div class="flex items-center justify-center w-9 h-9 rounded-lg {{ request()->routeIs('teacher-assessments*') ? 'bg-white/10' : 'bg-slate-800/50 group-hover:bg-slate-700/50' }} transition-all duration-200">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if(auth()->user()->hasRole('admin') || auth()->user()->hasRole('head-teacher') || auth()->user()->hasRole('deputy-head-teacher'))
            <!-- Academic Section -->
            <div class="pt-6 pb-2">
        ...
        @if(auth()->user()->isSuperAdmin())
            ...
            <div class="pt-4">
                <a href="{{ route('admin.features.index') }}" class="group flex items-center px-3 py-2.5 rounded-xl transition-all duration-200 {{ request()->routeIs('admin.features*') ? 'bg-gradient-to-r from-red-600 to-pink-500 text-white shadow-lg shadow-red-500/30' : 'text-slate-300 hover:bg-slate-800/50 hover:text-white' }}">
                    <div class="flex items-center justify-center w-9 h-9 rounded-lg {{ request()->routeIs('admin.features*') ? 'bg-white/10' : 'bg-slate-800/50 group-hover:bg-slate-700/50' }} transition-all duration-200">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
