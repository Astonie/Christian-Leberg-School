<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <h2 class="text-2xl font-bold text-gray-900">Add Timetable Entry</h2>
        </div>
    </x-slot>
    ...
    <div class="container-mobile section-spacing">
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden max-w-3xl mx-auto">
            ...
            <form action="{{ route('timetables.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div>
                        ...
                        <select name="academic_year_id" id="academic_year_id" required
                            ...
                            @foreach($academicYears as $year)
                                <option value="{{ $year->id }}" {{ old('academic_year_id', $preselected['academic_year_id']) == $year->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select name="term_id" id="term_id"
                            ...
                            @foreach($terms as $term)
                                <option value="{{ $term->id }}" {{ old('term_id', $preselected['term_id']) == $term->id ? 'selected' : '' }}>
                                    {{ $term->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="class_id" id="class_id" required
                            ...
                            @foreach($classes as $class)
                                <option value="{{ $class->id }}" {{ old('class_id', $preselected['class_id']) == $class->id ? 'selected' : '' }}>
                                    {{ $class->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="stream_id" id="stream_id"
                            ...
                            @foreach($streams as $stream)
                                <option value="{{ $stream->id }}" {{ old('stream_id', $preselected['stream_id']) == $stream->id ? 'selected' : '' }}>
                                    {{ $stream->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="day_of_week" id="day_of_week" required
                                class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                            @foreach($days as $day)
                                <option value="{{ $day }}" {{ old('day_of_week', $preselected['day_of_week']) == $day ? 'selected' : '' }}>
                                    {{ $day }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select name="period_id" id="period_id" required
                            ...
                            @foreach($periods as $period)
                                <option value="{{ $period->id }}" {{ old('period_id', $preselected['period_id']) == $period->id ? 'selected' : '' }}>
                    ...
                    <div class="md:col-span-2">
                        ...
                        <textarea name="notes" id="notes" rows="3"
                                  ...
                                  class="w-full px-4 py-2.5 border-2 border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-blue-500">{{ old('notes') }}</textarea>
                        @error('notes')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
