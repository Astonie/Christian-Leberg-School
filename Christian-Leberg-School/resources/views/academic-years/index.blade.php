            ...
            @if(session('warning'))
                <div class="mb-4 rounded-md bg-yellow-50 border border-yellow-200 p-4 text-yellow-800">
                    <div class="flex">
                        ...
                        <span class="font-semibold">Warning:</span> {{ session('warning') }}
                    </div>
                </div>
            @endif
            ...
            <div class="mb-4 flex justify-between items-center">
                ...
                <div>
                    @if($showArchived ?? false)
                        <a href="{{ route('academic-years.index') }}" class="text-gray-600 hover:text-gray-900 px-4 py-2 border border-gray-300 rounded">
                            ← Back to Active Years
                        </a>
                    @else
                        <a href="{{ route('academic-years.index', ['archived' => 1]) }}" class="text-gray-600 hover:text-gray-900 px-4 py-2 border border-gray-300 rounded">
                            View Archived Years
                        </a>
                    @endif
                </div>
                                ...
                                <tr class="{{ $year->is_active ? 'bg-green-50' : '' }}">
                                    <td class="px-6 py-4 whitespace-nowrap">
                                        ...
                                        @if($hasDependencies)
                                            <div class="text-xs text-gray-500 mt-1">
                                                {{ $streamCount }} streams, {{ $examCount }} exams, {{ $studentEnrollments }} enrollments
                                            </div>
                                        @endif
                                    </td>
                                    ...
                                    <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                        @if($showArchived ?? false)
                                            ...
                                            <form action="{{ route('academic-years.restore', $year->id) }}" method="POST" class="inline">
                                                @csrf
                                        ...
                                        @else
                                            ...
                                            @elseif($hasDependencies)
                                                <button 
                                                    type="button"
                                                    onclick="showDeleteWarning('{{ $year->name }}', {{ $streamCount }}, {{ $examCount }}, {{ $studentEnrollments }})"
