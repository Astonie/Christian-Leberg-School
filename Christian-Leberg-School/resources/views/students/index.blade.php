<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            @if(auth()->user()->hasRole(['admin', 'head-teacher', 'deputy-head-teacher']))
                <div class="flex flex-wrap items-center gap-2.5">
                    @if(auth()->user()->hasRole(['admin', 'head-teacher']))
                        <a href="{{ route('students.import.index') }}" class="inline-flex items-center px-4 py-2 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold rounded-lg shadow-md hover:shadow-lg transition-all duration-200">
                    ...
                    @endif
                    <a href="{{ route('students.create') }}" class="inline-flex items-center px-4 py-2 bg-gray-800 hover:bg-gray-900 text-white font-semibold rounded-lg shadow-sm transition-all duration-200">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8" x-data="{ view: '{{ request('view', 'cards') }}' }">
                ...
                <form method="GET" action="{{ route('students.index') }}" class="space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                        ...
                        <div class="md:col-span-2">
                            ...
                            <div class="relative">
                                <input type="text" id="search" name="search" value="{{ request('search') }}" placeholder="Name or admission number..." class="w-full pl-10 pr-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center space-x-2">
                            ...
                            <a href="{{ route('students.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors">
                                Clear
                            </a>
                        </div>
            ...
            @if($students->isEmpty())
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-12 text-center">
                    ...
                    @if(auth()->user()->hasRole('admin'))
                        <a href="{{ route('students.create') }}" class="inline-flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg">
                            <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path>
                            </svg>
                            Add Student
                        ...
                        <div class="bg-white rounded-xl shadow-sm border border-gray-200 hover:shadow-lg transition-shadow duration-200">
                            ...
                            <div class="bg-gray-700 p-4 rounded-t-xl">
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <h3 class="text-white font-semibold text-lg truncate">{{ $student->user->name }}</h3>
                                        <p class="text-gray-200 text-sm">{{ $student->admission_number }}</p>
                                    </div>
                                    <!-- Status Badge -->
                                    <div>
                            ...
                            <div class="p-4 space-y-3">
                                ...
                                <div class="flex items-center text-sm text-gray-600">
                                    ...
                                    @if($student->current_stream)
                                        <span class="font-medium text-gray-900">{{ optional($student->current_stream->schoolClass ?? null)->name ?? '-' }} - {{ optional($student->current_stream ?? null)->name ?? '-' }}</span>
                                    @else
                                ...
                                <div class="flex items-center text-sm text-gray-600">
                                    ...
                                    <span>Admitted: {{ $student->admission_date->format('M d, Y') }}</span>
                                </div>
                                ...
                                @if($student->date_of_birth)
                                    <div class="flex items-center text-sm text-gray-600">
                                        ...
                                        <span>Age: {{ \Carbon\Carbon::parse($student->date_of_birth)->age }} years</span>
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="border-t border-gray-100 px-4 py-3 bg-gray-50 rounded-b-xl">
                                <div class="flex items-center justify-between space-x-2">
                                    <a href="{{ route('students.show', $student) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors text-sm">
                                        <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    @if(auth()->user()->hasRole('admin'))
                                        <a href="{{ route('students.edit', $student) }}" class="flex-1 inline-flex items-center justify-center px-3 py-2 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium rounded-lg transition-colors text-sm">
                                            <svg class="w-4 h-4 mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        ...
                                        <button onclick="confirmDelete({{ $student->id }})" class="inline-flex items-center justify-center px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 font-medium rounded-lg transition-colors text-sm">
                                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path>
                                            </svg>
                                        </button>
                                        <form id="delete-form-{{ $student->id }}" action="{{ route('students.destroy', $student) }}" method="POST" class="hidden">
                                            @csrf
                                            @method('DELETE')
                                        </form>
                                    @endif
                                    ...
                                    @foreach ($students as $student)
                                        <tr class="hover:bg-gray-50 transition-colors">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                @if($student->current_stream)
                                                    <span class="font-medium">{{ optional($student->current_stream->schoolClass ?? null)->name ?? '-' }}</span>
                                                    <span class="text-gray-500">- {{ optional($student->current_stream ?? null)->name ?? '-' }}</span>
                                                @else
                                                    <span class="text-gray-400">Not Assigned</span>
                                                @endif
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-900">
                                                <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold {{ $student->gender === 'male' ? 'bg-blue-100 text-blue-800' : 'bg-pink-100 text-pink-800' }}">
                                                    {{ ucfirst($student->gender) }}
                                                </span>
                                            </td>
                                            <td class="px-6 py-4 whitespace-nowrap text-sm text-gray-600">{{ $student->admission_date->format('M d, Y') }}</td>
                                            <td class="px-6 py-4 whitespace-nowrap">
                                                @if($student->status === 'active')
                                                    <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-800">
                                                        <svg class="w-3 h-3 mr-1" fill="currentColor" viewBox="0 0 20 20">
                                            ...
                                            <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                <div class="flex items-center justify-end space-x-2">
                                                    <a href="{{ route('students.show', $student) }}" class="text-gray-600 hover:text-gray-900 font-medium">View</a>
                                                    @if(auth()->user()->hasRole('admin'))
                                                        <a href="{{ route('students.edit', $student) }}" class="text-gray-600 hover:text-gray-900 font-medium">Edit</a>
                                                        <button onclick="confirmDelete({{ $student->id }})" class="text-red-600 hover:text-red-900 font-medium">Delete</button>
                                                        <form id="delete-form-{{ $student->id }}" action="{{ route('students.destroy', $student) }}" method="POST" class="hidden">
                                                            @csrf
                                                            @method('DELETE')
                                                        </form>
