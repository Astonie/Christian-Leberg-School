    ...
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div class="flex items-center space-x-4">
                <a href="{{ route('classes.index') }}" class="text-gray-600 hover:text-gray-900 transition-colors">
                ...
                <div>
                    <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                        {{ $schoolClass->name }} - Stream Management
                    </h2>
                    <p class="text-sm text-gray-600 mt-1">Organize students into streams for {{ $activeYear->name ?? 'current year' }}</p>
                </div>
            </div>
        </div>
    </x-slot>
                ...
                <div class="rounded-xl bg-yellow-50 border border-yellow-200 p-6">
                    <div class="flex items-start">
                        ...
                        <div>
                            ...
                            <a href="{{ route('academic-years.create') }}" class="inline-flex items-center px-4 py-2 bg-yellow-600 hover:bg-yellow-700 text-white font-semibold rounded-lg transition-colors">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                        ...
                        <div class="p-6 border-b border-gray-200">
                            <div class="flex items-center justify-between">
                                <div>
                                    ...
                                    <p class="text-sm text-gray-600 mt-1">{{ $schoolClass->streams->count() }} stream(s) for {{ $activeYear->name ?? 'N/A' }}</p>
                                </div>
                                @if($hasActiveYear && $schoolClass->streams->count() > 0)
                                    ...
                                    <div class="p-6 hover:bg-gray-50 transition-colors">
                                        <div class="flex items-start justify-between">
                                            <div class="flex-1">
                                                <div class="flex items-center space-x-3 mb-3">
                                                    <div class="flex items-center justify-center w-10 h-10 bg-blue-100 rounded-lg">
                                                        <span class="text-blue-700 font-bold text-lg">{{ $stream->name }}</span>
                                                    </div>
                                                    <div>
                                                        <h4 class="text-base font-bold text-gray-900">{{ $stream->full_name }}</h4>
                                                ...
                                                <div class="mt-3 bg-gray-50 rounded-lg p-3 border border-gray-200">
                                                    <form method="POST" action="{{ route('streams.update', $stream) }}" class="flex items-center space-x-3">
                                                        @csrf
                                                        @method('PUT')
                                            ...
                                            <div class="ml-4 flex flex-col space-y-2">
                                                <button type="button"
                                                        @click="editStreamData = { id: {{ $stream->id }}, name: '{{ $stream->name }}', capacity: '{{ $stream->capacity ?? '' }}' }; editingStream = {{ $stream->id }}"
                                                ...
                                                <form action="{{ route('streams.destroy', $stream) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this stream? This action cannot be undone.')">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" 
                ...
                <div class="lg:col-span-1">
                    <div class="bg-white rounded-xl shadow-sm border border-gray-200 sticky top-6">
                        ...
                        <div class="p-6">
                            <form method="POST" action="{{ route('streams.store') }}" class="space-y-4">
                                @csrf
                                <input type="hidden" name="class_id" value="{{ $schoolClass->id }}">

                                <div>
                                    <x-input-label for="name" :value="__('Stream Name')" class="text-sm font-medium" />
                                ...
                                <div>
                                    <x-input-label for="capacity" :value="__('Capacity (Optional)')" class="text-sm font-medium" />
                                ...
                                <div class="pt-2">
                                    <x-primary-button class="w-full justify-center" :disabled="!$hasActiveYear">
                                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                                        </svg>
                                        {{ __('Add Stream') }}
