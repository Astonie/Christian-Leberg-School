        ...
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <div>
                ...
                <div class="flex items-center mt-2">
                    ...
                    <p class="text-sm text-gray-600 font-medium">{{ $year?->name ?? 'No Active Academic Year' }}</p>
                </div>
            </div>
            @if($subjects->isNotEmpty())
                <div class="flex flex-wrap items-center gap-2.5">
                    <a href="{{ route('teacher.streams') }}" class="inline-flex items-center px-4 py-2.5 bg-white border-2 border-gray-300 rounded-lg text-sm font-semibold text-gray-700 hover:bg-gray-50 hover:border-gray-400 transition-all duration-200 shadow-sm hover:shadow">
                    ...
                    <a href="{{ route('attendance.index') }}" class="inline-flex items-center px-4 py-2.5 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white rounded-lg text-sm font-semibold transition-all duration-200 shadow-md hover:shadow-lg transform hover:-translate-y-0.5">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-gradient-to-br from-blue-50 to-blue-100 border border-blue-200 rounded-2xl p-5 lg:p-6 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 cursor-pointer group">
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            ...
                            <p class="text-3xl sm:text-4xl font-bold text-blue-900 mb-1">{{ $statistics['total_subjects'] }}</p>
                ...
                <div class="bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-2xl p-5 lg:p-6 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 cursor-pointer group">
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            ...
                            <p class="text-3xl sm:text-4xl font-bold text-green-900 mb-1">{{ $statistics['total_students'] }}</p>
                ...
                <div class="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-2xl p-5 lg:p-6 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 cursor-pointer group">
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            ...
                            <p class="text-3xl sm:text-4xl font-bold text-purple-900 mb-1">{{ $statistics['total_streams'] }}</p>
                ...
                <div class="bg-gradient-to-br from-orange-50 to-orange-100 border border-orange-200 rounded-2xl p-5 lg:p-6 hover:shadow-xl hover:scale-[1.02] transition-all duration-300 cursor-pointer group">
                    <div class="flex items-center justify-between">
                        <div class="flex-1">
                            ...
                            <p class="text-3xl sm:text-4xl font-bold text-orange-900 mb-1">{{ $statistics['active_exams'] }}</p>
        ...
        @else
            <div>
                <div class="flex items-center justify-between mb-6">
                    ...
                    <span class="px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-semibold">{{ $subjects->count() }} {{ Str::plural('Subject', $subjects->count()) }}</span>
                </div>
                                    ...
                                    <div class="flex-1 min-w-0">
                                        <div class="flex items-center gap-4">
                                            <div class="w-16 h-16 sm:w-18 sm:h-18 bg-gradient-to-br from-blue-500 via-blue-600 to-blue-700 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg hover:scale-105 transition-transform duration-300">
                                                <span class="text-white font-bold text-xl sm:text-2xl">{{ strtoupper(substr($subject->code, 0, 2)) }}</span>
                                            </div>
                                            <div class="flex-1 min-w-0">
                                                <h3 class="text-lg sm:text-xl font-bold text-gray-900 mb-2 truncate">{{ $subject->name }}</h3>
                                                <div class="flex flex-wrap items-center gap-2">
                                                    ...
                                                    @if($subject->students_count > 0)
                                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-green-100 text-green-700 border border-green-200">
                                                            ...
                                                            {{ $subject->students_count }} {{ Str::plural('Student', $subject->students_count) }}
                                                        </span>
                                                    @else
                                                    ...
                                                    @endif
                                                    @if($subject->teacher_streams->isNotEmpty())
                                                        <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-semibold bg-purple-100 text-purple-700 border border-purple-200">
                                                            ...
                                                            {{ $subject->teacher_streams->count() }} {{ Str::plural('Stream', $subject->teacher_streams->count()) }}
                                                        </span>
                                                    @endif
                                                </div>
                                            </div>
                            ...
                            <div x-show="expanded" x-collapse class="border-t border-gray-100 bg-gray-50 px-5 lg:px-6 py-5">
                                @if($subject->description)
                                    <div class="mb-5">
                                        ...
                                        <p class="text-sm text-gray-700 leading-relaxed pl-6">{{ $subject->description }}</p>
                                    </div>
                                @endif
                                ...
                                @if($subject->teacher_streams->isNotEmpty())
                                    <div class="mb-5">
                                        <h4 class="text-sm font-bold text-gray-900 mb-3 flex items-center">
                                            ...
                                            Teaching Streams ({{ $subject->teacher_streams->count() }})
                                        </h4>
                                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                            @foreach($subject->teacher_streams as $stream)
                                                <div class="flex items-center space-x-3 px-4 py-3 bg-white rounded-xl border border-gray-200 hover:border-purple-300 hover:shadow-md transition-all duration-200">
                                                    ...
                                                    <div class="flex-1 min-w-0">
                                                        <p class="text-sm font-bold text-gray-900 truncate">{{ $stream->schoolClass->name }} - {{ $stream->name }}</p>
                                ...
                                <div>
                                    ...
                                    <div class="flex flex-wrap gap-2.5">
                                        ...
                                        @if($subject->students_count > 0)
                                            <a href="{{ route('students.index') }}?subject={{ $subject->id }}" class="inline-flex items-center px-4 py-2.5 bg-white border-2 border-gray-300 hover:border-purple-400 hover:bg-purple-50 text-gray-700 hover:text-purple-700 text-sm font-semibold rounded-xl transition-all duration-200 shadow-sm hover:shadow">
                                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 20h5v-2a3 3 0 00-5.356-1.857M17 20H7m10 0v-2c0-.656-.126-1.283-.356-1.857M7 20H2v-2a3 3 0 015.356-1.857M7 20v-2c0-.656.126-1.283.356-1.857m0 0a5.002 5.002 0 019.288 0M15 7a3 3 0 11-6 0 3 3 0 016 0zm6 3a2 2 0 11-4 0 2 2 0 014 0zM7 10a2 2 0 11-4 0 2 2 0 014 0z"></path>
                                                </svg>
                                                View Students
                                        ...
                                        @endif
                                        <a href="{{ route('attendance.reports') }}?subject={{ $subject->id }}" class="inline-flex items-center px-4 py-2.5 bg-white border-2 border-gray-300 hover:border-orange-400 hover:bg-orange-50 text-gray-700 hover:text-orange-700 text-sm font-semibold rounded-xl transition-all duration-200 shadow-sm hover:shadow">
                                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
