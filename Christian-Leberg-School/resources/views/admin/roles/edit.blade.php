                    ...
                    <form action="{{ route('admin.roles.update', $role) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" name="slug" id="slug" value="{{ old('slug', $role->slug) }}" 
                                   ...
                                   {{ in_array($role->slug, ['admin', 'teacher', 'student']) ? 'readonly' : '' }}
                                   required>
                            @if(in_array($role->slug, ['admin', 'teacher', 'student']))
                                <p class="mt-1 text-xs text-gray-500">System role slug cannot be changed</p>
                            @else
                        ...
                        <div class="mb-6">
                            ...
                            <textarea name="description" id="description" rows="3"
                                      class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-gray-400 focus:border-transparent">{{ old('description', $role->description) }}</textarea>
                            @error('description')
                                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                            @enderror
                        </div>
