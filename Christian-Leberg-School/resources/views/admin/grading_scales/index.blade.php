        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $totalScales }}</p>
                        </div>
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900 mt-1">{{ $systemsCount }}</p>
                        </div>
                        <div class="bg-gray-100 rounded-full p-3">
                            <svg class="w-8 h-8 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-3xl font-bold text-gray-900 mt-1">{{ number_format($avgPoints, 1) }}</p>
                        </div>
                        <div class="bg-gray-100 rounded-full p-3">
                            <svg class="w-8 h-8 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                ...
                @if($scales->count() > 0)
                    ...
                    @foreach($groupedScales as $systemId => $systemScales)
                        @php
                        ...
                        <div class="border-b border-gray-200 last:border-0">
                            <div class="bg-gray-50 px-6 py-4">
                                <div class="flex items-center justify-between">
                                    <div>
                                        <h4 class="font-semibold text-gray-900">{{ $system?->name ?? 'Default System' }}</h4>
                                        <p class="text-sm text-gray-600 mt-1">{{ $systemScales->count() }} grade levels configured</p>
                                    </div>
                            ...
                            <div class="overflow-x-auto">
                                <table class="min-w-full divide-y divide-gray-200">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-100">
                                        @foreach($systemScales->sortByDesc('min_percentage') as $scale)
                                            <tr class="hover:bg-gray-50 transition-colors">
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span class="inline-flex items-center justify-center w-10 h-10 rounded-lg font-bold text-lg
                                                        @if(($scale->min_percentage ?? 0) >= 80) bg-green-100 text-green-800
                                                        @elseif(($scale->min_percentage ?? 0) >= 70) bg-blue-100 text-blue-800
                                                        @elseif(($scale->min_percentage ?? 0) >= 60) bg-yellow-100 text-yellow-800
                                                        @elseif(($scale->min_percentage ?? 0) >= 50) bg-orange-100 text-orange-800
                                                        @else bg-red-100 text-red-800
                                                        @endif">
                                                        {{ $scale->code ?? substr($scale->label, 0, 1) }}
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <div class="flex items-center justify-center">
                                                        <span class="text-sm font-semibold text-gray-900">{{ $scale->min_percentage ?? $scale->min_score }}%</span>
                                                        <svg class="w-4 h-4 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 5l7 7m0 0l-7 7m7-7H3"></path>
                                                        </svg>
                                                        <span class="text-sm font-semibold text-gray-900">{{ $scale->max_percentage ?? $scale->max_score }}%</span>
                                                    </div>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap">
                                                    <span class="text-sm font-medium text-gray-900">{{ $scale->label }}</span>
                                                </td>
                                                <td class="px-6 py-4">
                                                    <span class="text-sm text-gray-600">{{ $scale->remark }}</span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-center">
                                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-semibold bg-gray-100 text-gray-900">
                                                        {{ $scale->grade_point ?? $scale->points }}
                                                    </span>
                                                </td>
                                                <td class="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                                                    <div class="flex items-center justify-end space-x-2">
                                                        <button onclick="editScale({{ $scale->id }}, '{{ $scale->min_percentage ?? $scale->min_score }}', '{{ $scale->max_percentage ?? $scale->max_score }}', '{{ $scale->label }}', '{{ $scale->remark }}', '{{ $scale->grade_point ?? $scale->points }}')" class="text-gray-600 hover:text-gray-900 font-medium">
                                                            Edit
                                                        </button>
                                                        <form action="{{ route('admin.grading_scales.destroy', $scale) }}" method="POST" onsubmit="return confirm('Are you sure you want to delete this grading scale?')">
                                                            @csrf
                                                            @method('DELETE')
            ...
            <div id="addScaleForm" class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                ...
                <form action="{{ route('admin.grading_scales.store') }}" method="POST" class="space-y-4">
                    @csrf
