<x-app-layout>
            ...
            <div class="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                <div class="flex items-center justify-between mb-6">
                    <div>
                        ...
                        <p class="text-sm text-gray-500 mt-1">{{ $system->scales->count() }} grades defined</p>
                    </div>
                        ...
                        <div class="flex items-center justify-between p-4 bg-gradient-to-r from-gray-50 to-white rounded-lg border border-gray-200 hover:shadow-md transition-shadow">
                            ...
                            <div class="flex items-center gap-4">
                                ...
                                <div class="flex gap-2">
                                    <button onclick="editScale({{ $scale->id }}, '{{ $scale->code }}', '{{ $scale->label }}', {{ $scale->min_score }}, {{ $scale->max_score }}, {{ $scale->points }}, {{ $scale->order ?? 0 }})" 
                                        class="p-2 text-blue-600 hover:bg-blue-50 rounded-lg transition-colors">
                                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path>
                                        </svg>
                                    </button>
                                    <form action="{{ route('admin.grading_scales.destroy', $scale) }}" method="POST" onsubmit="return confirm('Delete this grade scale?')">
                                        @csrf
                                        @method('DELETE')
    ...
    <div id="scaleModal" class="hidden fixed inset-0 bg-gray-500 bg-opacity-75 flex items-center justify-center z-50">
        <div class="bg-white rounded-xl shadow-xl max-w-2xl w-full mx-4">
            <form id="scaleForm" method="POST" action="{{ route('admin.grading_scales.store') }}">
                @csrf
                <input type="hidden" name="_method" id="formMethod" value="POST">
                <input type="hidden" name="grading_system_id" value="{{ $system->id }}">
    ...
    <script>
        function openAddScaleModal() {
            ...
            document.getElementById('scaleForm').action = '{{ route("admin.grading_scales.store") }}';
