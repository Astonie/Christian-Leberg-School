...
<head>
    ...
    <title>@yield('title', 'Welcome') - {{ setting('site_name', 'School Portal') }}</title>
        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="flex justify-between items-center h-16 md:h-20">
                ...
                <div class="flex-shrink-0 flex items-center gap-2 md:gap-3">
                    <a href="{{ route('website.home') }}" class="flex items-center gap-2 md:gap-3">
                        <img src="{{ asset('images/school_logo.png') }}" alt="CLSS Logo" class="w-10 h-10 md:w-12 md:h-12 object-contain">
                ...
                <div class="hidden md:flex items-center space-x-4 lg:space-x-6">
                    ...
                    @if($headerMenu)
                        @foreach($headerMenu->items as $item)
                            <a href="{{ $item->href }}" 
                               class="text-sm font-medium {{ $item->isActive() ? 'text-brand-600' : 'text-slate-600 hover:text-brand-600' }} transition-colors relative group">
                                {{ $item->title }}
            ...
            <div class="px-4 py-6">
                ...
                <div class="space-y-2">
                    @if($headerMenu)
                        @foreach($headerMenu->items as $item)
                            <a href="{{ $item->href }}" 
                               class="block px-4 py-3 rounded-lg text-base font-medium {{ $item->isActive() ? 'bg-brand-50 text-brand-600' : 'text-slate-700 hover:bg-slate-50' }} transition-colors">
                                {{ $item->title }}
                            </a>
                        @endforeach
                    @endif
                    <a href="{{ route('login') }}" class="block px-4 py-3 mt-4 bg-slate-900 text-white rounded-lg text-base font-medium hover:bg-slate-800 transition-all text-center">
                        Portal Login
                    </a>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12 mb-8 md:mb-12">
                <div class="col-span-1 sm:col-span-2">
                    <div class="flex items-center gap-3 mb-4 md:mb-6">
                        <img src="{{ asset('images/school_logo.png') }}" alt="CLSS Logo" class="w-12 h-12 md:w-14 md:h-14 object-contain">
                        <h3 class="text-lg md:text-xl font-bold text-white">{{ setting('site_name', 'School Portal') }}</h3>
                    </div>
                    <p class="text-slate-300 text-sm md:text-base leading-relaxed mb-4 md:mb-6">{{ setting('site_description', 'Excellence in Education - Building futures through quality education') }}</p>
                    <div class="flex gap-3 md:gap-4">
                        @if(setting('social_facebook'))
                            <a href="{{ setting('social_facebook') }}" target="_blank" rel="noopener" class="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-lg flex items-center justify-center transition-colors">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z"/></svg>
                            </a>
                        @endif
                        @if(setting('social_twitter'))
                            <a href="{{ setting('social_twitter') }}" target="_blank" rel="noopener" class="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-lg flex items-center justify-center transition-colors">
                                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z"/></svg>
                            </a>
                        @endif
                        @if(setting('social_instagram'))
                            <a href="{{ setting('social_instagram') }}" target="_blank" rel="noopener" class="w-10 h-10 bg-white/10 hover:bg-white/20 rounded-lg flex items-center justify-center transition-colors">
                ...
                <div>
                    ...
                    <ul class="space-y-2 md:space-y-3">
                        <li><a href="{{ route('website.home') }}" class="text-slate-300 hover:text-white transition-colors text-sm md:text-base hover:translate-x-1 inline-block">Home</a></li>
                        <li><a href="{{ route('website.blog') }}" class="text-slate-300 hover:text-white transition-colors text-sm md:text-base hover:translate-x-1 inline-block">News & Blog</a></li>
                        <li><a href="{{ route('website.events') }}" class="text-slate-300 hover:text-white transition-colors text-sm md:text-base hover:translate-x-1 inline-block">Events</a></li>
                    </ul>
                </div>
                <div>
                    ...
                    <ul class="space-y-2 md:space-y-3 text-slate-300 text-sm md:text-base">
                        <li class="flex items-start gap-2">
                            ...
                            <span class="break-words">{{ setting('contact_address', 'Malawi') }}</span>
                        </li>
                        <li class="flex items-center gap-2">
                            <svg class="w-5 h-5 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                            <span>{{ setting('contact_phone', '+265 xxx xxx xxx') }}</span>
                        </li>
                        <li class="flex items-center gap-2">
                            ...
                            <span class="break-all">{{ setting('contact_email', 'info@clss.mw') }}</span>
                        </li>
                    </ul>
                </div>
            </div>
            <div class="border-t border-slate-700/50 pt-6 md:pt-8 text-center">
                <p class="text-slate-300 text-sm md:text-base">&copy; {{ date('Y') }} <span class="font-semibold text-white">{{ setting('site_name', 'School Portal') }}</span>. All rights reserved.</p>
            </div>
        </div>
    </footer>
