        ...
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6">
                ...
                <div class="mt-4">
                    ...
                    <a href="{{ route('exams.results.create', $exam) }}" class="ml-2 px-3 py-2 bg-blue-600 text-white rounded hover:bg-blue-700">Enter Results</a>
                    @if(auth()->user()->hasRole('teacher'))
                        @foreach(auth()->user()->teacher->subjects()->wherePivot('academic_year_id', $exam->academic_year_id)->get() as $subj)
                            <a href="{{ route('exams.results.create_for_subject', [$exam, $subj]) }}" class="ml-2 px-3 py-2 bg-blue-500 text-white rounded hover:bg-blue-600">Enter {{ $subj->name }} Results</a>
                        @endforeach
                    @endif
                </div>
            </div>
