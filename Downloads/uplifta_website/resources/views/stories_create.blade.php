...
<div class="container mx-auto py-12 max-w-xl">
    <h1 class="text-2xl font-bold mb-6">Add Story</h1>
    <form method="POST" action="{{ route('stories.store') }}" class="bg-white p-8 rounded shadow space-y-6">
        @csrf
        <div>
        ...
        <div>
            ...
            <input type="text" name="country" class="w-full border rounded px-3 py-2" value="{{ old('country') }}">
        </div>
        <div>
            <label class="block font-semibold mb-1">Quote</label>
            <textarea name="quote" class="w-full border rounded px-3 py-2" required>{{ old('quote') }}</textarea>
        </div>
        <div>
