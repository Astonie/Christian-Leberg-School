...
<div class="container mx-auto py-12 max-w-xl">
    ...
    <form method="POST" action="{{ route('services.store') }}" class="bg-white p-8 rounded shadow space-y-6">
        @csrf
        <div>
        ...
        <div>
            ...
            <textarea name="features[]" class="w-full border rounded px-3 py-2" rows="3">{{ old('features.0') }}</textarea>
        </div>
        <div>
            <label class="block font-semibold mb-1">CTA Label</label>
            <input type="text" name="cta_label" class="w-full border rounded px-3 py-2" value="{{ old('cta_label') }}">
        </div>
        <div>
            <label class="block font-semibold mb-1">CTA Link</label>
            <input type="text" name="cta_link" class="w-full border rounded px-3 py-2" value="{{ old('cta_link') }}">
        </div>
